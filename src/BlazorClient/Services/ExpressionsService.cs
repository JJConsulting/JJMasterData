...
public class ExpressionsService : IExpressionsService
{
    ...
    private HttpContext CurrentContext { get; }
        ...
        ExpressionManagerInterval interval = null)
    {
        ...
        foreach (string field in list)
        {
            ...
            {
                val = $"{CurrentContext.Request.Query["objname"]}";
            }
            else if (
                CurrentContext.Session != null &&
                CurrentContext.Session.TryGetValue(field, out var _))
            {
                val = CurrentContext.Session.GetString(field);
            }
            else
            {
                val = "";
