...
internal sealed class TextFileScripts(JJTextFile textFile)
{
    public string GetShowScript()
    {
        ...
        return $"TextFileHelper.showUploadView('{textFile.FieldName}','{title}','{textFile.DataProtectionService.ProtectObject(routeContext)}','{textFile.GetDraftInputName()}');";
    }
    ...
    public string GetRefreshScript()
    {
        ...
        return $"TextFileHelper.refresh('{textFile.FieldName}','{textFile.DataProtectionService.ProtectObject(routeContext)}','{textFile.GetDraftInputName()}')";
    }
